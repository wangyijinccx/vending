	...
	public void daTask() {
		...
		if (list != null && list.size() > 0) {
			for (ClusterGameAccount clusterGameAccount : list) {
				...
				String url_scr = "http://XXXXX/dms/game_scripts" + "?id="
						+ clusterGame.getBundleid() + "&version="
						+ clusterGame.getVersion() + "&platform="
						+ clusterGame.getPlatform();
				logger.info("adcluster-url_scr:{}", url_scr);
				String callback_scr = httpService.get(url);
				...
				// 执行任务
				String url_do = "http://XXXXX/dms/run_script?udid=" + udid
						+ "&internal_id=" + internal_id + "&script_id="
						+ script_id + "&game_id=" + clusterGame.getBundleid()
						+ "&username=" + username + "&password=" + password
						+ "&server=" + server + "&time=" + time + "&recover=";
				logger.info("adcluster-url_do:{}", url_do);
				String doback = httpService.get(url);
